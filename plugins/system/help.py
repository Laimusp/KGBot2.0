...
@KGBot.on_message(filters.me & filters.command('help', KGBot.prefix))
...
async def get_modules_help(module_name: str, app: KGBot, system: bool = False) -> dict:
    ...
    module = import_module(f'plugins.{module_name}') if not system else importlib.import_module(f'plugins.system.{module_name}')
